...
Calculate distance correlation between protein abundance and multiple target variables with FDR BH correction.
...
| `target_cols` | Target Columns | text | Yes | - | Always visible |
...
| `num_resamples` | Permutation Resamples | number (min: 99, max: 999, step: 100) | No | 199 | Always visible |
...
Comma-separated list of numeric column names from annotation to correlate with.
...
#### Permutation Resamples (`num_resamples`)

Number of resamples for permutation test p-value calculation.
...
| `volcano_plot` | `volcano_plot.html` | html | html | Volcano plot showing distance correlation vs significance. |
...
| `ranked_correlations` | `ranked_correlations.html` | html | html | Bar plot showing proteins ranked by distance correlation. |
...
1. Navigate to **statistics** → **Distance Correlation Analysis**
...
```typescript
const jobId = await pluginService.executePlugin('distance-correlation', {
  // Add parameters here
});
```
